...
const ChatContainer = ({ selectedUser, setSelectedUser }) => {
  ...
  useEffect(()=>{
    if(scrollEnd.current){
      scrollEnd.current.scrollIntoView({behvior:"smooth"})
    }
  },[])
  return selectedUser ? (
          ...
          <div
            ...
            {msg.image ? (
              <img
                src={msg.image}
                alt=""
                className="max-w-[230px] border border-gray-700 round-lg overflow-hidden mb-8"
              />
            ) : (
              <p
                className={`p-2 max-w-[200px] md:text-sm font-light rounded-1g mb-8 break-all bg-violet-500/30 text-white $(msg.senderId === '680f50e4f10f3cd28382ecf9'? 'rounded-br-none': 'rounded-bl-none'}`}
              >
                {msg.text}
              </p>
            )}
...
<label htmlFor="image">
  <img src={assets.gallery_icon} alt="" className="w-5 mr-2 cursor-pointer"/>
    ...
    <div className="flex flex-col items-center justify-center gap-2 text-gray-500 bg-white/10 max-md:hidden">
      ...
      <p className="text-lg front-medium text-white">
        {" "}
